locals {
  # Derive project/env prefix from cluster_id if needed — service_name is enough for naming
  full_name = "enheritage-${var.service_name}"
}
  ...
  container_definitions = jsonencode([
    {
      ...
      healthCheck = {
        command     = ["CMD-SHELL", "curl -sf http://localhost:${var.port}/health || exit 1"]
        interval    = 30
        timeout     = 5
        retries     = 3
        startPeriod = 60
